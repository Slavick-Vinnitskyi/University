...
ShapeObjectsEditor choose_shape = ShapeObjectsEditor();
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
		...
		case IDM_ABOUT:
			...
			break;
		case ID_DOT:
			choose_shape.StartPointEditor();
			SetWindowText(hWnd, L"DOT");
			break;
		case ID_LINE:
			choose_shape.StartLineEditor();
			SetWindowText(hWnd, L"LINE");
			break;
		case ID_RECTANGLE:
			choose_shape.StartRectEditor();
			SetWindowText(hWnd, L"RECTANGLE");
			break;
		case ID_ELLIPSE:
			choose_shape.StartEllipseEditor();
	...
	break;
	case WM_LBUTTONDOWN:
		choose_shape.OnLBdown(hWnd, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
		break;
	case WM_LBUTTONUP:
		choose_shape.OnLBup(hWnd, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
		break;
	case WM_MOUSEMOVE:
		choose_shape.OnMouseMove(hWnd, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
		break;
	case WM_PAINT:
		choose_shape.OnPaint(hWnd);
		break;
	case WM_DESTROY:
		PostQuitMessage(0);
		break;
