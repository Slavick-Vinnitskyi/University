...
int max_page_calls = 10; // Max number of page refs to test
...
Page_Ref* gen_ref()
{
        Page_Ref *page = ((Page_Ref *)malloc(sizeof(Page_Ref)));
...
{
        Frame *framep = ((Frame *)malloc(sizeof(Frame)));
        framep->index = index;
        framep->page = -1;
        framep->r_bit = 0;
        return framep;
...
int add_victim(struct Frame_List *victim_list, struct Frame *frame)
{
        ...
        struct Frame *victim = ((Frame *)malloc(sizeof(Frame)));
        *victim = *frame;
        victim->index = 1;
